...
void LGF_RGB2HSL(CGFloat r, CGFloat g, CGFloat b,
...
void LGF_HSL2RGB(CGFloat h, CGFloat s, CGFloat l,
...
void LGF_RGB2HSB(CGFloat r, CGFloat g, CGFloat b,
...
void LGF_HSB2RGB(CGFloat h, CGFloat s, CGFloat v,
                CGFloat *r, CGFloat *g, CGFloat *b) {
    CLAMP_COLOR_VALUE(h);
    CLAMP_COLOR_VALUE(s);
    CLAMP_COLOR_VALUE(v);
...
void LGF_RGB2CMYK(CGFloat r, CGFloat g, CGFloat b,
...
void LGF_CMYK2RGB(CGFloat c, CGFloat m, CGFloat y, CGFloat k,
                 CGFloat *r, CGFloat *g, CGFloat *b) {
    CLAMP_COLOR_VALUE(c);
    CLAMP_COLOR_VALUE(m);
    CLAMP_COLOR_VALUE(y);
...
void LGF_HSB2HSL(CGFloat h, CGFloat s, CGFloat b,
...
void LGF_HSL2HSB(CGFloat h, CGFloat s, CGFloat l,
...
+ (UIColor *)lgf_ColorWithHue:(CGFloat)hue
               saturation:(CGFloat)saturation
                lightness:(CGFloat)lightness
                    alpha:(CGFloat)alpha {
    CGFloat r, g, b;
    LGF_HSL2RGB(hue, saturation, lightness, &r, &g, &b);
...
+ (UIColor *)lgf_ColorWithCyan:(CGFloat)cyan
                   magenta:(CGFloat)magenta
                    yellow:(CGFloat)yellow
                     ...
                     alpha:(CGFloat)alpha {
    CGFloat r, g, b;
    LGF_CMYK2RGB(cyan, magenta, yellow, black, &r, &g, &b);
    return [UIColor colorWithRed:r green:g blue:b alpha:alpha];
}
...
static inline NSUInteger lgf_HexStrToInt(NSString *str) {
    uint32_t result = 0;
    sscanf([str UTF8String], "%X", &result);
    return result;
}

static BOOL lgf_HexStrToRGBA(NSString *str,
                         CGFloat *r, CGFloat *g, CGFloat *b, CGFloat *a) {
    
    str = [[str lgf_StringByTrim] uppercaseString];
    if ([str hasPrefix:@"#"]) {
    ...
    //RGB,RGBA,RRGGBB,RRGGBBAA
    if (length < 5) {
        *r = lgf_HexStrToInt([str substringWithRange:NSMakeRange(0, 1)]) / 255.0f;
        *g = lgf_HexStrToInt([str substringWithRange:NSMakeRange(1, 1)]) / 255.0f;
        *b = lgf_HexStrToInt([str substringWithRange:NSMakeRange(2, 1)]) / 255.0f;
        if (length == 4)  *a = lgf_HexStrToInt([str substringWithRange:NSMakeRange(3, 1)]) / 255.0f;
        else *a = 1;
    } else {
        *r = lgf_HexStrToInt([str substringWithRange:NSMakeRange(0, 2)]) / 255.0f;
        *g = lgf_HexStrToInt([str substringWithRange:NSMakeRange(2, 2)]) / 255.0f;
        *b = lgf_HexStrToInt([str substringWithRange:NSMakeRange(4, 2)]) / 255.0f;
        if (length == 8) *a = lgf_HexStrToInt([str substringWithRange:NSMakeRange(6, 2)]) / 255.0f;
        else *a = 1;
    }
    return YES;
}

+ (instancetype)lgf_ColorWithHexString:(NSString *)hexStr {
    CGFloat r, g, b, a;
    if (lgf_HexStrToRGBA(hexStr, &r, &g, &b, &a)) {
        return [UIColor colorWithRed:r green:g blue:b alpha:a];
    }
    return nil;
}

+ (instancetype)lgf_ColorWithHexString:(NSString *)hexStr alpha:(CGFloat)alpha {
    CGFloat r, g, b, a;
    if (lgf_HexStrToRGBA(hexStr, &r, &g, &b, &a)) {
        return [UIColor colorWithRed:r green:g blue:b alpha:alpha];
    }
...
- (NSString *)lgf_HexString {
    return [self lgf_HexStringWithAlpha:NO];
}

- (NSString *)lgf_HexStringWithAlpha {
    return [self lgf_HexStringWithAlpha:YES];
}

- (NSString *)lgf_HexStringWithAlpha:(BOOL)withAlpha {
...
- (UIColor *)lgf_ColorByChangeHue:(CGFloat)h saturation:(CGFloat)s brightness:(CGFloat)b alpha:(CGFloat)a {
    CGFloat hh, ss, bb, aa;
...
- (BOOL)lgf_GetHue:(CGFloat *)hue
    ...
    LGF_RGB2HSL(r, g, b, hue, saturation, lightness);
    *alpha = a;
    return YES;
}
...
- (BOOL)lgf_GetCyan:(CGFloat *)cyan
    ...
    LGF_RGB2CMYK(r, g, b, cyan, magenta, yellow, black);
    *alpha = a;
    return YES;
}
...
- (CGColorSpaceModel)lgf_ColorSpaceModel {
    return CGColorSpaceGetModel(CGColorGetColorSpace(self.CGColor));
}

- (NSString *)lgf_ColorSpaceString {
