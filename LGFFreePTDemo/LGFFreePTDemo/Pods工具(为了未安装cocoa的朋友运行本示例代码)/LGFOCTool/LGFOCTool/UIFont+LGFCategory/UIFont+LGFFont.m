...
+ (UIFont *)lgf_FontWithCGFont:(CGFontRef)CGFont size:(CGFloat)size {
...
+ (UIFont *)lgf_LoadFontFromData:(NSData *)data {
...
+ (NSData *)lgf_DataFromFont:(UIFont *)font {
    CGFontRef cgFont = font.lgf_CGFontRef;
    NSData *data = [self lgf_DataFromCGFont:cgFont];
    CGFontRelease(cgFont);
    return data;
}

typedef struct lgf_FontHeader {
    int32_t fVersion;
    uint16_t fNumTables;
...
static uint32_t lgf_CalcTableCheckSum(const uint32_t *table, uint32_t numberOfBytesInTable) {
    uint32_t sum = 0;
...
+ (NSData *)lgf_DataFromCGFont:(CGFontRef)cgFont {
    if (!cgFont) return nil;
    ...
    size_t totalSize = sizeof(lgf_FontHeader) + sizeof(lgf_TableEntry) * tableCount;
    ...
    lgf_FontHeader *offsetTable = (lgf_FontHeader *)dataPtr;
    ...
    dataPtr += sizeof(lgf_FontHeader);
    
    // write tables
    lgf_TableEntry *entry = (lgf_TableEntry *)dataPtr;
    dataPtr += sizeof(lgf_TableEntry) * tableCount;
    ...
    for (int index = 0; index < tableCount; ++index) {
        ...
        entry->fCheckSum = CFSwapInt32HostToBig(lgf_CalcTableCheckSum((uint32_t *)dataPtr, (uint32_t)tableSize));
